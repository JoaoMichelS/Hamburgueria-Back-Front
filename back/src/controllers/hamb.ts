...
class HambController {
  ...
  async create(req: Request, res: Response) {
    const { name, ingredientes, preco, estoque } = req.body;
    try {
      const hamb = await Hamb.create({ name, ingredientes, preco, estoque }); // Cria no banco
  ...
  async update(req: Request, res: Response) {
    ...
    try {
      await Hamb.findByIdAndUpdate(
        id,
        { name, ingredientes, preco, estoque },
        { new: true }
      ).then((hamb) => {
        if (hamb) {
      ...
      if (hamb) {
        if (hamb.estoque > 0) {
          if (hamb.preco > +dinheiro) {
            ...
            await Hamb.findByIdAndUpdate(id, { name: hamb.name, ingredientes: hamb.ingredientes, preco: hamb.preco, estoque: hamb.estoque - 1 });
            res.status(200).json({
